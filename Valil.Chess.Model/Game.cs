...
{
    /// <summary>
    /// Implements a chess game. Powered by Valil Chess Library
    /// </summary>
    public class ValilGame : Component
    {
        ...
        internal TimeSpan GetTimeSpent(DateTime gameStart, bool isWhite = true)
        {
            DateTime lastMoveTime = gameStart;
            ...
            foreach (var move in moveHistory)
            {
                ...
                if (skip)
                {
                    lastMoveTime = move.MakeTime;
                    continue;
                }
                result += move.MakeTime - lastMoveTime;
                lastMoveTime = move.MakeTime;
            }

            if (skip && moveHistory.Count > 0)
            {
                result += DateTime.Now - moveHistory.FindLast(x => true).MakeTime;
            }

            return result;
        }

        public TimeSpan GetWhiteTimeSpent(DateTime gameStart)
        {
            return GetTimeSpent(gameStart, true);
        }

        public TimeSpan GetBlackTimeSpent(DateTime gameStart)
        {
            return GetTimeSpent(gameStart, false);
        }

        internal TimeSpan GetTimeLeft(DateTime gameStart, TimeSpan initialtime, TimeSpan addTimePerMove, bool isWhite)
        {
            var timeSpent = GetTimeSpent(gameStart, isWhite);
            var timeAdded = new TimeSpan(0, 0, 0, (int)(addTimePerMove.TotalSeconds * ((moveHistory.Count + (isWhite ? 1 : 0)) / 2)), 0);
            var timeLeft = initialtime - timeSpent + timeAdded;
            return timeLeft;
        }

        public TimeSpan GetWhiteTimeLeft(DateTime gameStart, TimeSpan initialtime, TimeSpan addTimePerMove)
        {
            var timeLeft = GetTimeLeft(gameStart, initialtime, addTimePerMove, true);
            return timeLeft;
        }

        public TimeSpan GetBlackTimeLeft(DateTime gameStart, TimeSpan initialtime, TimeSpan addTimePerMove)
        {
            var timeLeft = GetTimeLeft(gameStart, initialtime, addTimePerMove, false);
            return timeLeft;
        }
        ...
        public ValilGame(IContainer container)
            : this()
        {
            container.Add(this);
        }
